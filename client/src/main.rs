...
fn load_cfg(client_config: &String) -> Result<ClientConfig> {
    ...
    let kedolik_cp_program_str = config.get("Global", "kedolik_cp_program").unwrap();
    if kedolik_cp_program_str.is_empty() {
        panic!("kedolik_cp_program must not be empty");
    }
    let kedolik_cp_program = Pubkey::from_str(&kedolik_cp_program_str).unwrap();
...
pub struct Opts {
    #[clap(subcommand)]
    pub command: KedolikCpCommands,
}

#[derive(Debug, Parser)]
pub enum KedolikCpCommands {
    InitializePool {
        mint0: Pubkey,
        mint1: Pubkey,
        init_amount_0: u64,
...
fn main() -> Result<()> {
    ...
    let program = anchor_client.program(pool_config.kedolik_cp_program)?;

    let opts = Opts::parse();
    match opts.command {
        KedolikCpCommands::InitializePool {
            mint0,
            mint1,
            init_amount_0,
            init_amount_1,
        ...
        KedolikCpCommands::Deposit {
            pool_id,
            user_token_0,
            user_token_1,
            lp_token_amount,
        } => {
            let pool_state: kedolik_cp_swap::states::PoolState = program.account(pool_id)?;
            // load account
            ...
            // docode account
            let pool_state =
                utils::deserialize_anchor_account::<kedolik_cp_swap::states::PoolState>(
            ...
            let (total_token_0_amount, total_token_1_amount) = pool_state
                ...
                .unwrap();
            // calculate amount
            let results = kedolik_cp_swap::curve::CurveCalculator::lp_tokens_to_trading_tokens(
                u128::from(lp_token_amount),
                u128::from(pool_state.lp_supply),
                u128::from(total_token_0_amount),
                u128::from(total_token_1_amount),
                kedolik_cp_swap::curve::RoundDirection::Ceiling,
            )
            .ok_or(kedolik_cp_swap::error::ErrorCode::ZeroTradingTokens)
            .unwrap();
            println!(
        ...
        KedolikCpCommands::SwapBaseIn {
            ...
            let pool_state =
                ...
                .unwrap();
            let amm_config_state = deserialize_anchor_account::<kedolik_cp_swap::states::AmmConfig>(
            ...
            let (
                ...
                (
                    kedolik_cp_swap::curve::TradeDirection::ZeroForOne,
                ...
                (
                    kedolik_cp_swap::curve::TradeDirection::OneForZero,
                    total_token_1_amount,
                    total_token_0_amount,
                    user_input_token,
                    spl_associated_token_account::get_associated_token_address(
            ...
            let result = kedolik_cp_swap::curve::CurveCalculator::swap_base_input(
        ...
        KedolikCpCommands::SwapBaseOut {
            ...
            let result = kedolik_cp_swap::curve::CurveCalculator::swap_base_output(
        ...
        KedolikCpCommands::DecodeInstruction { instr_hex_data } => {
            handle_program_instruction(&instr_hex_data, InstructionDecodeType::BaseHex)?;
        }
        KedolikCpCommands::DecodeEvent { log_event } => {
            handle_program_log(
                &pool_config.kedolik_cp_program.to_string(),
                &log_event,
                false,
            )?;
        }
        KedolikCpCommands::DecodeTxLog { tx_id } => {
            ...
            // decode logs
            parse_program_event(&pool_config.kedolik_cp_program.to_string(), meta.clone())?;
        }
    }
    Ok(())
}
