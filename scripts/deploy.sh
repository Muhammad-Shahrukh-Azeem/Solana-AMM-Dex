...
if [ "$NETWORK" == "mainnet" ]; then
    ...
    echo -e ""
    echo -e "   2. Set up KEDOLOG (if needed):"
    echo -e "      ${GREEN}NETWORK=mainnet npx ts-node scripts/config/setup-kedolog.ts${NC}"
    echo -e ""
    echo -e "   3. Create pools and test thoroughly before announcing!"
else
    echo -e "\n${YELLOW}🎯 Next Steps (Devnet):${NC}"
    echo -e "   1. Configure KEDOLOG:"
    echo -e "      ${GREEN}./scripts/quick-setup-kedolog.sh YOUR_KEDOLOG_MINT${NC}"
    echo -e ""
